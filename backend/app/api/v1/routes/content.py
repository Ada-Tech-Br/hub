...
def list_contents(
    ...
    result = content_service.list_contents(db, page=page, page_size=page_size, search=search, type=type)
...
def get_content(content_id: uuid.UUID, db: DBSession, current_user: CurrentUser):
    return ContentResponse.model_validate(content_service.get_content_by_id(db, content_id))


@router.patch("/{content_id}", response_model=ContentResponse)
def update_content(content_id: uuid.UUID, data: ContentUpdate, db: DBSession, admin: AdminUser):
...
async def upload_file(
    ...
    if filename.endswith(".zip"):
        content = content_service.handle_zip_upload(db, content_id, file_bytes, updated_by=admin.id)
    elif filename.endswith(".html") or filename.endswith(".htm"):
        content = content_service.handle_html_upload(db, content_id, file_bytes, filename, updated_by=admin.id)
    else:
